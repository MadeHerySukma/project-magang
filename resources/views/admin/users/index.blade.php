        ...
        <div class="container mx-auto px-6 py-3">
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-2">
                    ...
                    <h1 class="text-lg font-bold">Kelola Pengguna</h1>
                </div>
                            <!-- Desktop Menu -->
    ...
    <div class="container mx-auto px-6 py-4">
        ...
        <div class="bg-gradient-to-r from-blue-900 to-blue-800 rounded-lg shadow-md p-4 text-white mb-4">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-blue-200 text-sm">Lihat dan kelola semua pengguna sistem</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div class="bg-white border border-blue-200 rounded-lg shadow-md hover:shadow-lg p-4 transition">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-blue-950">{{ $totalUsers }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-br from-green-50 to-green-100 border border-green-300 rounded-lg shadow-md hover:shadow-lg p-4 transition">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-green-700 text-xs font-bold uppercase mb-1">Pelanggan</p>
                        <p class="text-2xl font-bold text-green-600">{{ $totalPelanggan }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-300 rounded-lg shadow-md hover:shadow-lg p-4 transition">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-purple-700 text-xs font-bold uppercase mb-1">Admin</p>
                        <p class="text-2xl font-bold text-purple-600">{{ $totalAdmin }}</p>
                    </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-md p-4 mb-4">
            <div class="flex items-center mb-3">
                ...
                <h3 class="text-sm font-bold text-gray-900">Cari & Filter Pengguna</h3>
            </div>
            
            <form method="GET" action="{{ route('admin.users.index') }}">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div class="md:col-span-2">
                        <label class="block text-gray-900 font-bold mb-1 text-xs uppercase">Cari Pengguna</label>
                        <input type="text" name="search" value="{{ request('search') }}" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" 
                            placeholder="Cari nama atau email...">
                    </div>
                    <div>
                        <label class="block text-gray-900 font-bold mb-1 text-xs uppercase">Role</label>
                        <select name="role" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                ...
                <div class="flex gap-2 mt-3">
                    ...
                    <a href="{{ route('admin.users.index') }}" class="flex-1 bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg text-sm font-bold text-center transition flex items-center justify-center">
                        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gradient-to-r from-blue-900 to-blue-800 text-white">
                        <tr>
                            <th class="px-4 py-3 text-left font-bold text-xs uppercase">#</th>
                            <th class="px-4 py-3 text-left font-bold text-xs uppercase">Nama & Email</th>
                            <th class="px-4 py-3 text-left font-bold text-xs uppercase">Role</th>
                            <th class="px-4 py-3 text-left font-bold text-xs uppercase">Bergabung</th>
                            <th class="px-4 py-3 text-left font-bold text-xs uppercase">Statistik</th>
                            <th class="px-4 py-3 text-center font-bold text-xs uppercase">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @forelse($users as $user)
                        <tr class="user-row hover:bg-blue-50 transition">
                            <td class="px-4 py-3">
                                <span class="text-sm font-bold text-blue-950">{{ $loop->iteration }}</span>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <div class="h-10 w-10 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 flex items-center justify-center text-white font-bold shadow-md">
                                            {{ strtoupper(substr($user->name, 0, 1)) }}
                                        </div>
                                    </div>
                                    <div class="ml-3">
                                        <div class="text-sm font-bold text-blue-950">{{ $user->name }}</div>
                            ...
                            <td class="px-4 py-3">
                                @if($user->role == 'admin')
                                    <span class="px-2 py-1 inline-flex text-xs font-bold rounded-lg bg-purple-100 text-purple-800">
                                ...
                                @else
                                    <span class="px-2 py-1 inline-flex text-xs font-bold rounded-lg bg-green-100 text-green-800">
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"/>
                                        </svg>
                                        Pelanggan
                            ...
                            <td class="px-4 py-3">
                                <div class="text-sm font-semibold text-blue-950">{{ $user->created_at->format('d M Y') }}</div>
                                <div class="text-xs text-gray-500">{{ $user->created_at->diffForHumans() }}</div>
                            </td>
                            <td class="px-4 py-3">
                                @php
                                    $totalPemesanan = \App\Models\Pemesanan::where('user_id', $user->id)->count();
                                    $totalBelanja = \App\Models\Pemesanan::where('user_id', $user->id)
                                        ->whereIn('status', ['paid', 'confirmed', 'active', 'completed'])
                                        ->sum('total_harga');
                                @endphp
                                <div class="text-sm font-semibold text-blue-950">{{ $totalPemesanan }} Pemesanan</div>
                                <div class="text-xs text-green-600 font-bold">Rp {{ number_format($totalBelanja, 0, ',', '.') }}</div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center gap-1 flex-wrap">
                                    <a href="{{ route('admin.users.show', $user->id) }}" 
                                        class="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition inline-flex items-center">
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <a href="{{ route('admin.users.edit', $user->id) }}" 
                                        class="bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition inline-flex items-center">
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <form method="POST" action="{{ route('admin.users.destroy', $user->id) }}" class="inline">
                                        ...
                                        <button type="submit" onclick="return confirm('Yakin ingin menghapus user ini?')"
                                            class="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition inline-flex items-center">
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center">
                                ...
                                <p class="text-gray-600 text-base font-bold mb-1">Tidak ada user ditemukan</p>
        ...
        <div class="mt-4 bg-white border border-gray-200 rounded-lg shadow-md p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-gray-700">
                <div class="flex items-start">
                    ...
                    <p><strong>Detail:</strong> Klik untuk melihat detail user dan riwayat transaksi</p>
                </div>
                <div class="flex items-start">
                    <svg class="w-4 h-4 text-yellow-600 mr-2 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793z"/>
                    </svg>
                    <p><strong>Edit:</strong> Klik untuk mengubah data user (nama, email, role, password)</p>
                </div>
                <div class="flex items-start">
                    ...
                    <p><strong>Hapus:</strong> Hapus user yang tidak memiliki pemesanan aktif</p>
                </div>
                <div class="flex items-start">
                    <svg class="w-4 h-4 text-yellow-600 mr-2 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92z" clip-rule="evenodd"/>
                    </svg>
                    <p><strong>Catatan:</strong> Tidak dapat menghapus akun sendiri atau user dengan pemesanan aktif</p>
                </div>
            </div>
        </div>
    </div>
